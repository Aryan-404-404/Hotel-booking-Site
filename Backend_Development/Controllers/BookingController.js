...
const updateBooking = asyncHandler(async (req, res) => {
    ...
    await updatedBooking.populate('user')
...
const updateField = asyncHandler(async (req, res) => {
    try {
        ...
        await sendMail(
            ...
            `
            <div style="font-family: Arial, sans-serif; color: #333; padding: 20px;">
              <h2 style="color: #2E8B57;">Booking Status Updated</h2>
              <p>Hi <strong>${updatedBooking.user.userName}</strong>,</p>
              <p>Your room booking has been updated. It is now scheduled from <strong>${new Date(updatedBooking.checkIn).toLocaleDateString()}</strong> to <strong>${new Date(updatedBooking.checkOut).toLocaleDateString()}</strong>.</p>
              <p>Details:</p>
              <ul>
                <li><strong>Guests:</strong> ${updatedBooking.persons}</li>
                <li><strong>Status:</strong> ${updatedBooking.status}</li>
              </ul>
              <p>Thank you for choosing us!</p>
            </div>
            `
